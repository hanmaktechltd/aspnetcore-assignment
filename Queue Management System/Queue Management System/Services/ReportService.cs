    ...
    public byte[] GenerateReport()
{
    try
    {
        ...
        DisplayTextObject(dataBand, "AverageServiceTimeHeadline", 0, dataBand.Height, Units.Millimeters * 100, Units.Millimeters * 5, "AVG Service Times:", 8, FontStyle.Bold);
        dataBand.Height += Units.Millimeters * 5;

        Dictionary<string, double> averageServiceTimes = FindAverageServiceTimePerServiceProvider();
        if (averageServiceTimes.Count == 0)
        {
           
            DisplayTextObject(dataBand, "NoFinishedTicketsMessage", 0, dataBand.Height, Units.Millimeters * 100, Units.Millimeters * 5, "No finished tickets at the moment");
            dataBand.Height += Units.Millimeters * 10;
        }
        else
        {
            foreach (var kvp in averageServiceTimes)
            {
                string serviceProviderName = kvp.Key;
                double avgServiceTimeSeconds = kvp.Value;

                int serviceMinutes = (int)avgServiceTimeSeconds / 60;
                int serviceSeconds = (int)avgServiceTimeSeconds % 60;

                string formattedAvgServiceTime = $"{serviceMinutes} min {serviceSeconds} sec";

                DisplayTextObject(dataBand, $"{serviceProviderName}ServiceName", 0, dataBand.Height, Units.Millimeters * 50, Units.Millimeters * 5, serviceProviderName);
                DisplayTextObject(dataBand, $"{serviceProviderName}AvgServiceTime", Units.Millimeters * 50, dataBand.Height, Units.Millimeters * 50, Units.Millimeters * 5, formattedAvgServiceTime);
    ...
    private Dictionary<string, double> FindAverageWaitingTimePerServicePoint()
    {
        ...
        using (var connection = new NpgsqlConnection(_connectionString))
        {
            ...
            using (var command = new NpgsqlCommand("SELECT ServicePoint, AVG(EXTRACT(EPOCH FROM (ServiceStartTime - IssueTime))) AS AvgWaitTime " +
                                                   "FROM Ticket " +
    ...
    private Dictionary<string, double> FindAverageServiceTimePerServiceProvider()
    {
        Dictionary<string, double> averageServiceTimes = new Dictionary<string, double>();

        using (var connection = new NpgsqlConnection(_connectionString))
        {
            connection.Open();

            using (var command = new NpgsqlCommand("SELECT ServiceProvider, AVG(EXTRACT(EPOCH FROM (ServiceCompleteTime - ServiceStartTime))) AS AvgServiceTime " +
                                                   "FROM Ticket " +
                                                   "WHERE Status = 'Finished' " +
                                                   "GROUP BY ServiceProvider", connection))
            {
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        string serviceProvider = reader["ServiceProvider"].ToString();
                        double avgServiceTime = Convert.ToDouble(reader["AvgServiceTime"]);

                        averageServiceTimes.Add(serviceProvider, avgServiceTime);
                    }
                }
            }
        }

        return averageServiceTimes;
    }
}
